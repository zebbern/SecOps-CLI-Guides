...
version: 1.0.0
tags: [windows, privilege-escalation, post-exploitation, lateral-movement, security-testing]
---
...
### Issue: Exploit Fails Due to AV Detection
**Cause**: Antivirus blocking known exploits
**Solution**:
```powershell
# Check AV product
WMIC /Node:localhost /Namespace:\\root\SecurityCenter2 Path AntivirusProduct Get displayName
...
# Options:
# 1. Use obfuscated/custom compiled exploits
# 2. Use living-off-the-land techniques
# 3. Disable AV if permissions allow (not recommended)

# Use built-in Windows tools instead
# Example: mshta, certutil, bitsadmin
```

### Issue: Service Won't Start After Modification
**Cause**: Binary path syntax or dependencies
**Solution**:
```powershell
# Ensure correct syntax with spaces
sc config MyService binpath= "C:\path\to\binary.exe"
...
### Issue: Token Impersonation Fails
**Cause**: Wrong privilege or Windows version
**Solution**:
```powershell
# Verify privileges
whoami /priv

# Check Windows version for compatible exploit
systeminfo | findstr /B /C:"OS Name" /C:"OS Version"

# JuicyPotato: Windows 10 1809 and below, Server 2019 and below
# PrintSpoofer: Windows 10, Server 2016/2019
# GodPotato: Windows 8-11, Server 2012-2022
```

### Issue: Cannot Find Kernel Exploit
**Cause**: System fully patched or wrong architecture
**Solution**:
```powershell
...
# Use Windows Exploit Suggester for accurate matching
python wes.py --update
python wes.py systeminfo.txt
```

### Issue: PowerShell Script Blocked
**Cause**: Execution policy or AMSI
**Solution**:
```powershell
# Bypass execution policy
powershell -ep bypass -c "IEX(command)"

# Alternative: Use cmd equivalents
# Or: Encode script in base64
powershell -enc <base64_encoded_command>

# Use alternative tools that don't rely on PowerShell
```
