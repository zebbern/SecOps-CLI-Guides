...
version: 1.0.0
tags: [shodan, reconnaissance, osint, vulnerability-discovery, network-scanning]
---
...
### Issue: "Query Credits Exhausted"
**Cause**: Monthly credits consumed
**Solution**:
```bash
# Check remaining credits
shodan info

# Use credit-free queries (no filters)
shodan search apache  # Free
shodan count openssh  # Free

# Wait for monthly reset or upgrade plan
```

### Issue: "Host Has Been Recently Crawled"
**Cause**: Cannot re-scan same IP within 24 hours
**Solution**:
```bash
# Use existing data instead
shodan host IP_ADDRESS

# Or wait 24 hours for new scan
# Enterprise plans allow more frequent scanning
```

### Issue: "Rate Limit Exceeded"
**Cause**: More than 1 request per second
**Solution**:
```python
import time
...
shodan search 'org:"Company Name"'  # Correct
shodan search org:Company Name      # Wrong

# Broaden search criteria
shodan search 'hostname:example.com'  # Instead of specific subdomain

# Check filter spelling
shodan search 'product:nginx'  # Not 'Product:nginx'
```

### Issue: Downloaded File Won't Parse
**Cause**: Corrupted download or wrong format
**Solution**:
```bash
# Verify file integrity
gunzip -t results.json.gz

# Re-download with explicit format
shodan download --limit 100 results.json.gz "query"
